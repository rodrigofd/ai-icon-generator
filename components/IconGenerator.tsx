...
import { generateIcons, generateReferencedIcons, getStyleDescription as getBaseStyleDescription, getSafeMaskColor, editImage } from '../services/geminiService'
...
const IconGenerator = () => {
  ...
  const [isPaidModel, setIsPaidModel] = useState<boolean>(false)

  useEffect(() => {
    setIsPaidModel(selectedModel === 'gemini-3.1-flash-image-preview' || selectedModel === 'gemini-3-pro-image-preview')
  }, [selectedModel])

  const checkAndPromptForKey = async () => {
    if (selectedModel === 'gemini-3.1-flash-image-preview' || selectedModel === 'gemini-3-pro-image-preview') {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (formRef.current) {
        ...
        // When the bottom of the form is near the top of the viewport, or passed it
        // We use a threshold to trigger it "progressively" as the user scrolls past the form
        const threshold = 60
        if (rect.bottom < threshold) {
          setShowCompactForm(true)
        } else {
          setShowCompactForm(false)
          setIsCompactSettingsOpen(false) // Close settings if we go back to full form
        }
      }
    }
  ...
  const generateFullPrompt = useCallback((promptForIcon: string) => {
    ...
    const paddingInstruction = padding > 0
      ? "Constraint: Ensure distinct separation from the canvas edges (i.e. with a padding of approx. " + padding + "px)."
    ...
    const hasExternalRefs = uploadedImages.length > 0

    if (referenceIcon || hasExternalRefs) {
      // If we have uploaded images, or referenceIcon is in INSPIRE mode
      if (hasExternalRefs || (referenceIcon && referenceIcon.mode === 'inspire')) {
        return `${systemPreamble} Mode: INSPIRATION. Task: Create a NEW ${is3D ? 'asset' : 'icon'} for: "${promptForIcon}". Constraint: Strictly copy the artistic style, lighting, and rendering technique of the provided reference image(s). ${backgroundInstruction} ${negativePrompt}`
      }
      // If we only have referenceIcon and it is in EDIT mode
      else if (referenceIcon && referenceIcon.mode === 'edit') {
  ...
  }, [style, color, isUiIcon, padding, referenceIcon, isSingleColorStyle, uploadedImages])
  ...
  const handleSetReference = useCallback((iconId: string, mode: ReferenceMode) => {
    ...
    if (icon) {
      ...
      setUploadedImages([]) // Clear uploads when selecting from history to avoid confusion
      setStyle(icon.style)
      setColor(icon.color)
  ...
  const processFiles = useCallback((files: FileList | null) => {
    ...
    Promise.all(readers).then(() => {
      if (newImages.length > 0) {
        ...
        setReferenceIcon(null) // Clear historical reference if uploading new ones
        setToast({ message: `${newImages.length} image(s) attached for inspiration.` })
      }
    })
  }, [])
  ...
  const handlePaste = useCallback((e: ClipboardEvent) => {
    if (e.clipboardData && e.clipboardData.items) {
      ...
      if (files.length > 0) {
        e.preventDefault()
        // Manually construct a FileList-like object or just reuse logic
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Check for API key if using a paid model
    ...
    if (window.innerWidth < 1024) {
      // If in compact mode, we might already be scrolled down.
      // Scroll to top of results if not already there, or stay put.
      // If we are at the top, we should probably scroll to results.
    ...
    // Clean Base64 strings (remove prefix if exists)
    const cleanedReferenceImages = referenceImagesList.map(img => {
  ...
  }, [prompt, style, numVariants, isUiIcon, color, referenceIcon, uploadedImages, customPrompt, padding, isBatchMode, generateFullPrompt, selectedModel])
  ...
  return (
    <div className="relative min-h-[80vh]">
      <Portal>
        ...
        {/* Compact Form Sticky Header (Mobile Only) */}
        <div className={`fixed top-0 left-0 right-0 z-40 transform transition-all duration-300 ease-in-out lg:hidden ${showCompactForm ? 'translate-y-0 shadow-lg' : '-translate-y-full shadow-none'}`}>
            ...
            <form onSubmit={handleSubmit} className="max-w-6xl mx-auto relative">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`p-2.5 rounded-full border transition-all ${isCompactSettingsOpen ? 'bg-[var(--color-accent)] text-white border-transparent' : 'bg-[var(--color-surface-secondary)] text-[var(--color-text-dim)] border-[var(--color-border)] hover:text-[var(--color-text)]'}`}
                >
              ...
              {/* Collapsible Settings Panel in Compact Mode */}
                ...
                <div className="p-3 bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] shadow-sm space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex-1 flex items-center gap-2 px-3 py-2 bg-[var(--color-surface-secondary)] rounded-lg border border-[var(--color-border)]">
                      ...
                      <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="w-6 h-6 rounded cursor-pointer border-none bg-transparent" />
                    </div>
                    <div className="flex-1 flex items-center justify-between px-3 py-2 bg-[var(--color-surface-secondary)] rounded-lg border border-[var(--color-border)]">
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 4].map(n => (
                          <button key={n} type="button" onClick={() => setNumVariants(n)} className={`w-6 h-6 text-xs rounded flex items-center justify-center ${numVariants === n ? 'bg-[var(--color-text)] text-[var(--color-bg)]' : 'text-[var(--color-text-dim)]'}`}>{n}</button>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-6">
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
              {/* Prompt Card */}
              <div
                className={`relative bg-[var(--color-surface)] border rounded-2xl p-1 shadow-sm transition-all duration-300 focus-within:shadow-md focus-within:border-[var(--color-accent)] ${isFileDragging ? 'border-2 border-dashed border-[var(--color-accent)] ring-4 ring-[var(--color-accent)]/10 scale-[1.02] bg-[var(--color-accent-glow)]' : 'border-[var(--color-border)]'}`}
                  ...
                  <div className={`transition-all duration-300 overflow-hidden ${isAdvancedOpen ? 'max-h-[400px] opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                    <div className="space-y-4 p-4 bg-[var(--color-surface-secondary)] rounded-xl border border-[var(--color-border)]">
                      ...
                      <div>
                        ...
                        {isPaidModel && (
                          <div className="mt-2 flex items-center justify-between p-2 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                            <span className="text-[10px] font-medium text-blue-600">Paid model requires a selected API key.</span>
                            <button
                              type="button"
              ...
              <button type="submit" disabled={isLoading} className="w-full py-4 rounded-2xl font-bold text-white text-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-70 disabled:transform-none disabled:shadow-none transition-all duration-200 flex justify-center items-center gap-3"
                ...
                {isLoading ? <><Spinner /> Generating...</> : (referenceIcon || uploadedImages.length > 0 ? 'Regenerate Icon' : 'Generate Icons')}
              </button>
            </form>
          </div>
