...
const callGenerationApi = async (prompt: string, numVariants: number, modelName: string, referenceImagesB64?: string[]) => {
  // FIX: Support multiple reference images by mapping them to inlineData parts
  const parts: any[] = [];
...
export const generateReferencedIcons = async (
  ...
  try {
    return await callGenerationApi(generationPrompt, numVariants, model, referenceImagesB64);
  } catch (error) {
...
// Backward compatibility alias (optional but good practice if referenced elsewhere temporarily)
export const generateReferencedIcon = async (
  ...
  return generateReferencedIcons(generationPrompt, numVariants, [referenceImageB64], model);
};
